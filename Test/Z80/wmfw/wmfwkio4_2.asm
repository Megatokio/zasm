...
TWEAK_SIZE  equ	12		; Byte count for each volume in table
...
Label_3:
    ...
    ld      (0d52bh),hl
    ld      hl,07d0h
    ld      (0d54ch),hl		; Maximum Volume
    ld      hl,0001h		; Bill changed this 2 -> 1
    ld      (0d54eh),hl		; Minimum Volume
    ld      hl,0000h
...
Label_110:
    ld      ix,Table_12		; 12-column move stage data table
    ld      hl,(0de50h)
...
Label_111:
    ...
    ld      (0de56h),hl
Label_112:
    ld      ix,Table_24		; 24-column move stage data table
    ld      hl,(0de50h)
    ld      (0dde3h),hl		; Set current column to (de50)
    add     hl,hl
    push    hl
...
Label_171:
    ...
    ld      a,00h
    ld      (0d54bh),a
    jp      Label_150		; Jump to top of volume state
...
L_DisVolSub:
    ...
    sub     a
    ld      h,a
    call    L_DisVol_1		; Display HL register
    ld      hl,1000
    ld      (0d562h),hl
    call    L_Delay
    pop     hl
...
L_DisVol_1:
    ...
    ld      a,(0d59eh)
    ld      (0d513h),a
    call    Label_336		; Program port 80 w/ 81 = 98, LED Display
    ret     
...
;				; Bill's stuff
    ld      a,1
    ld      (0de5dh), a		; Tweak index, @ offset 0 is len of its vol
    ld      hl,Tweaktable
    ld      bc,(0d040h)
    ld      a,19		; Max volume in Tweaktable
    cp      c
    jp      m,Label_267b	; Jump and use vol 0 for vols not in Tweaktable
    ld      b,c
Label_267a:
    ld      e,(hl)
    ld      a,0
    ...
    djnz    Label_267a
Label_267b:
    ld      (0de5eh),hl		; Tweak base
;
    call    Label_465		; uL to pulse conversion (* 40)
    ret     
...
;	START function - state 6

Label_282:			; Pump Fluid
    call    Label_283
    ld      a,(0d532h)
    inc     a
    ld      (0d532h),a
...
Label_283:
    ...
    call    Label_366		; Look at pump and pump-jaw
    ld      a,00h
    ld      (0d567h),a		; Set direction
    ld      hl,(0d003h)
    ld      (0d586h),hl		; Set speed
;				; Bill's stuff
    ld      hl,(0de5eh)		; Get tweak_base
    ld      a,(0de5dh)		; Get tweak_index
    ld      c,a
    cp      (hl)
    jp      nz,Label_283z	; Jump if not at end of entries for this vol
    sub     a			; Reset to beginning of entries for this vol
Label_283z:
    inc     a
    ld      (0de5dh),a		; Inc tweak_index & put back
    ld      b,0
    add     hl,bc
...
;
    ...
    ld      (0d57bh),a		; Set
    or      a
    jp      nz,Label_283c	; If pump volume is zero: Don't run pump.
    ld      a,h
    or      l
    ret     z
...
Tweaktable:
    ...
    defb     -8,  4, 14, 0, -11, -10
    defb     12					; 3 uL
    defb     -18, -8, 20, -16, -16, 20
    defb     -4, -16,  4,  12, -16, -12
    defb     12					; 4 uL
    defb     -28, 20,  -16,  -20, 20, -20
    defb     4,   0,  -20, 20,  -20,  -4
    defb     2					; 5 uL
    defb     -20, 20 
    defb     12					; 6 uL
    defb     -20, 8, 8,  -20, 16,  -16
    defb    12, 8, -20, 12,  -16, 12
    defb     12					; 7 uL
    defb       -4, -18, 14,  6, -20, 4
    defb     16, -16,   -4, 12,  0, -16
    defb     12					; 8 uL
    defb     5, -25,   -2, 10, 14, 10
    defb     -20,  -4, 10, 14,  8,  -18
    defb     12					; 9 uL
    defb     2,  -1,  -10, -13,  -8, 3
    defb     3, 3, 11, 5, 5,  -4
    defb      1					; 10 uL
