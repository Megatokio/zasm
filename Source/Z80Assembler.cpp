...
void Z80Assembler::checkCpuOptions()
{
	...
	if (target_z80)
	{
		if (ixcbr2_enabled && ixcbxh_enabled) throw FatalError("options ixcbr2 and ixcbxh are mutually exclusive.");
	}
...
void Z80Assembler::assembleFile(
	cstr sourcefile, cstr destpath, cstr listpath, cstr temppath, int liststyle, int deststyle, bool clean) noexcept
{
	...
	sourcefile = fullpath(sourcefile, no);
	assert(errno == ok && is_file(sourcefile));
	if (destpath)
	{
		destpath = fullpath(destpath);
		assert(errno == ok || errno == ENOENT);
	}
	if (listpath)
	{
		listpath = fullpath(listpath);
		assert(errno == ok || errno == ENOENT);
	}
	if (temppath)
	{
		temppath = fullpath(temppath);
		assert(errno == ok && is_dir(temppath));
	}
	...
	if (convert_8080)
	{
		...
		if (endswith(z80_file, "/"))
			z80_file = catstr(destpath, basename, ".z80"); // modifying the extension only simplifies usage in zasm.cgi
	...
	try
	{
		...
		if (source.count() && startswith(source[0], "#!"))
		{
			...
			if (cpu == CpuDefault && find(s, "--8080"))
			{
				cpu = Cpu8080;
				m	= " --8080";
			}
			if (cpu == CpuDefault && find(s, "--z180"))
			{
				cpu = CpuZ180;
				m	= " --z180";
			}
			...
			if (!ixcbr2_enabled && !ixcbxh_enabled && find(s, "--ixcbr2"))
			{
				ixcbr2_enabled = yes;
				m			   = catstr(m, " --ixcbr2");
			}
			if (!ixcbr2_enabled && !ixcbxh_enabled && find(s, "--ixcbxh"))
			{
				ixcbxh_enabled = yes;
				m			   = catstr(m, " --ixcbxh");
			}
			if (!syntax_8080 && find(s, "--asm8080"))
			{
				syntax_8080 = yes;
				m			= catstr(m, " --asm8080");
			}
			if (!allow_dotnames && find(s, "--dotnames"))
			{
				allow_dotnames = yes;
				m			   = catstr(m, " --dotnames");
			}
			if (!require_colon && find(s, "--reqcolon"))
			{
				require_colon = yes;
				m			  = catstr(m, " --reqcolon");
			}
			if (!casefold && find(s, "--casefold"))
			{
				casefold = yes;
				m		 = catstr(m, " --casefold");
			}
			if (!flat_operators && find(s, "--flatops"))
			{
				flat_operators = yes;
				m			   = catstr(m, " --flatops");
			}
		...
		if (errors.count() == 0 && deststyle)
		{
			...
			else { writeTargetfile(destpath, deststyle); }
		}
	}
	catch (AnyError& e)
	{
...
void Z80Assembler::setLabelValue(Label* label, cValue& value) { setLabelValue(label, value.value, value.validity); }
...
void Z80Assembler::setLabelValue(Label* label, int32 value, Validity validity)
{
	if (label->segment == nullptr)
	{
		...
		if (label->sourceline == 0) label->sourceline = current_sourceline_index;
	...
	if (label->is_redefinable)
	{
		...
		if (current_sourceline_index > label->sourceline) label->was_redefined = yes;
		goto x;
	}
	...
	if (validity > label->value.validity) labels_resolved++;
...
x:
	label->value	  = Value(value, validity);
...
void Z80Assembler::assemble(StrArray& sourcelines, cstr sourcepath) noexcept
{
	...
	source.purge();
	for (uint i = 0; i < sourcelines.count(); i++)
	{
		source.append(new SourceLine(sourcepath, i, dupstr(sourcelines[i])));
	}
	...
	if (pass > 99)
	{
		setError("internal error: pass > 99");
		return;
	} // 'labels_resolved' not properly updated
	if (validity == invalid)
	{
		setError("some labels failed to resolve");
		return;
	}
	...
	if (labels_changed > 0)
	{
		setError("some labels failed to settle");
		return;
	}
	...
	for (uint i = 0; i < segments.count(); i++)
	{
		...
		flags.append(s.address.validity);
		s.address.validity = valid;
		flags.append(s.size.validity);
		s.size.validity = valid;
...
void Z80Assembler::assembleOnePass(uint pass) noexcept
{
	...
	if (cond[0] != no_cond)
	{
		setError("#endif missing");
		return;
	} // TODO: set error marker in '#if/#elif/#else' line
	assert(!cond_off);
	if (local_labels_index != 0)
	{
		setError("#endlocal missing");
		return;
	} // TODO: set error marker in '#local' line
	...
	try
	{
		...
		for (uint i = 0; i < segments.count(); i++)
		{
			DataSegment* s = dynamic_cast<DataSegment*>(segments[i].ptr());
			if (!s) continue;
...
void Z80Assembler::skip_expression(SourceLine& q, int prio)
{
	...
	else
	{
		// multi-char word:
		if (w[0] == '$' || w[0] == '%' || w[0] == '\'' || w[0] == '&')
			goto op; // hex number or $$, binary, hex, or ascii number
	}
	...
	{
		if (lceq(w, "defined") || lceq(w, "hi") || lceq(w, "lo") || lceq(w, "min") || lceq(w, "max") ||
			lceq(w, "abs") || lceq(w, "opcode") || lceq(w, "target") || lceq(w, "segment") || lceq(w, "required") ||
...
op:
	...
	case '^':
		if (pBits <= prio) break;
		skip_expression(++q, pBits);
		goto op;

	case '?':
		if (pTriadic <= prio) break;
		skip_expression(++q, pTriadic - 1);
		q.expect(':');
		skip_expression(q, pTriadic - 1);
		goto op;
...
w:
	...
	else // multi-char word:
	{
		...
		if (w[0] == '$') // hex number or $$
		{
			...
			{
			hex_number:
				while (is_hex_digit(*w))
				{
					n.value = (n.value << 4) + (*w & 0x0f);
					if (*w > '9') n.value += 9;
					w++;
				}
				if (w[c != 0] == 0)
	...
	if (is_dec_digit(w[0])) // decimal number or reusable label
	{
		...
		{
			while (is_dec_digit(*w))
			{
				n.value = n.value * 10 + *w - '0';
				w++;
			}
			if (*w == 0) goto op;
	...
	{
		n = int(q.sourcelinenumber); /* valid = valid && yes; */
		goto op;
	}
	...
	if (q.test_char('(')) // test for built-in function
	{
		...
		{						// note: label value is not neccessarily valid
			...
			if (pass == 1)
			{
				...
				for (uint i = global ? 0 : local_labels_index;; i = labels[i].outer_index)
				{
					...
					if (label != nullptr && label->is_defined)
					{
						n = 1;
						break;
					}
			...
			else { n = if_values[if_values_idx++]; }
			goto kzop;
		}
		...
		{						 // note: label value is not neccessarily valid
			...
			if (pass == 1)
			{
				...
				for (uint i = global ? 0 : local_labels_index;; i = labels[i].outer_index)
				{
					...
					if (label != nullptr)
					{
						n = label->is_used && !label->is_defined;
						break;
					}
					if (i == 0)
					{
		...
		else if (lceq(w, "sin") || lceq(w, "cos")) // sin(rot,fullrot,maxval)
		{
			...
			if (abs(b.value) < 4)
			{
				if (b.is_valid()) throw SyntaxError("value for full circle must be ≥ 4");
				b.value = 360;
			}
			q.expectComma();
			...
			if (n.value == 0)
			{
				if (n.is_valid()) throw SyntaxError("scale value for result must be ≥ 1");
				n.value = 128;
			}
		...
		else if (lceq(w, "target"))
		{
			...
			n = target_ext != nullptr ? q.testWord(target_ext) :
				default_target != BIN ? q.testWord("rom") :
										q.testWord("ram") || q.testWord("bin");
...
op:
	...
	if (q.testEol()) goto x;
	c1 = q.p[0];
	if (is_uppercase(c1)) c1 |= 0x20;
	c2 = q.p[1];
	if (is_uppercase(c2)) c2 |= 0x20;
	...
	case pTriadic: // ?:
		if (c1 == '?')
		{
			...
			*/
			...
			{
				Value a = value(q, pTriadic - 1);
				q.expect(':');
				Value b	   = value(q, pTriadic - 1);
...
x:
	this->validity = min(n.validity, this->validity);
	return n;
}
	...
	else // program label, SET or MACRO or no label
	{
		if (!is_reusable) // SET and MACRO don't require ':'
		{
			...
			if (q.testDotWord("set")) // and the M80 pseudo instruction SET
				if (!doteq(
						name,
						"macro")) // "macro set ..." => opt for macro definition. this macro name may make trouble :-)
...
a:
	...
	{
		if (pass == 1 &&
			!syntax_8080) // 8080 all names allowed: mnenonic decides which arg is a register and which is a value
		{
...
void Z80Assembler::asmCompress(SourceLine& q)
{
	...
	CodeSegment* s1 = dynamic_cast<CodeSegment*>(l1->segment);
	assert(s1);
	...
	if (multiple) // compress range of segments
	{
		...
		CodeSegment* s2 = dynamic_cast<CodeSegment*>(l2->segment);
		assert(s2);
		...
		uint a;
		for (a = 0; ne(segments[a]->name, name); a++) {} // must exist
		uint e;
		for (e = 0; ne(segments[e]->name, name2); e++) {} // must exist
	...
	{
	a:
		if (s1->compressed == single_cseg) return; // duplicate directive
	...
	for (uint i = 1 - multiple; i < 4; i++)
	{
		...
		Label* l  = global_labels().find(n1);
		if (l && l->is_defined) throw SyntaxError("label %s redefined", n1);
		if (!l)
			global_labels().add(new Label(n1, current_segment_ptr, current_sourceline_index, 0, invalid, yes, yes, no));
		else
		{
			l->is_defined = yes;
			l->segment	  = current_segment_ptr;
			l->sourceline = current_sourceline_index;
		}
	}
}
...
void Z80Assembler::compressSegments()
{
	...
	for (uint i = 0; !compr && i < segments.count(); i++)
		if (auto s = dynamic_cast<CodeSegment*>(segments[i].ptr())) compr = s->compressed;
	if (!compr) return;
	...
	for (uint i = 0; i < segments.count();)
	{
		...
		// set labels:
		if (multiple) setLabelValue(global_labels().find(catstr(name, "_size")), usize);
...
void Z80Assembler::asmDefine(SourceLine& q)
{
	...
	if (l) { setLabelValue(l, n); }
	else
	{
		if (pass == 1 &&
			!syntax_8080) // 8080 all names allowed: mnenonic decides which arg is a register and which is a n
		{
...
uint Z80Assembler::skipMacroBlock(uint idx, cstr macro, cstr endm)
{
	...
	*/
	...
	for (idx += 1; idx < source.count(); idx += 1)
	{
		...
		if (doteq(w, "rept")) { idx = skipMacroBlock(idx, w, ".endm"); }
		else if (doteq(w, "dup")) { idx = skipMacroBlock(idx, w, ".edup"); }
		else if (doteq(w, "macro")) { idx = skipMacroBlock(idx, w, ".endm"); }
	}
...
void Z80Assembler::asmRept(SourceLine& q, cstr rept, cstr endm)
{
	...
	*/
	...
	if (pass == 1)
	{
		...
		{
			n = N1;
			setError("number of repetitions missing");
		}
		else
		{
			...
			{
				n = N1;
				setError("count must evaluate in pass 1");
			}
			else if (n.value > 0x8000)
			{
				n = N1;
				setError("number of repetitions too high");
			}
			else if (n.value < 0)
			{
				n = N1;
				setError("number of repetitions negative");
			}
		}
	}
	else
	...
	if (source.count() + uint32(n) * (e - a - 1) > 1000000) throw FatalError("total source exceeds 1,000,000 lines");

	RCArray<SourceLine> zsource;
	while (n.value--)
	{
		for (uint32 i = a + 1; i < e; i++) { zsource.append(new SourceLine(source[i])); }
	}
...
void Z80Assembler::asmMacro(SourceLine& q, cstr macro, cstr name, char tag)
{
	...
	*/
	...
	macros.add(
		name, Macro(std::move(args), a, e, tag, required)); // note: args[] & name are unprotected cstr in tempmem!
}
	...
	if (!q.testEol()) do
		{
			if (q.testChar('<')) // extended argument: < ... " ... ' ... , ... ; ... > [,;\n]
			{
				...
				do {
					while (*q && *q != '>') { ++q; }
					if (*q == 0) throw SyntaxError("closing '>' missing");
					ae = q.p;
					++q; // skip '>'
				}
				while (!q.testEol() && *q != ',');
...
void Z80Assembler::asmCFlags(SourceLine& q)
{
	...
	if (pass > 1)
	{
		q.skip_to_eol();
		return;
	} // c-source was compiled in pass 1!
	...
	while (!q.testEol())
	{
		...
		{								// -Ior/path/rel/to/source/dir	=> path in #cflags is relative to source file!
			...
			path = fullpath(path);
			if (errno) throw FatalError(errno);
...
void Z80Assembler::asmInclude(SourceLine& q)
{
	...
	if (is_library)
	{
		...
		else { fqn = get_directory(q); }

		Array<cstr> names;
		if (q.testWord("resolve") && !q.testChar('*'))
			for (;;)
			{
				cstr w = q.nextWord();
		...
		for (uint i = 0; i < files.count(); i++)
		{
			...
			if (endswith(fname, ".c") || endswith(fname, ".cc") || endswith(fname, ".s") || endswith(fname, ".ass") ||
				endswith(fname, ".asm"))
			{
	...
	else
	{
		...
		if (endswith(fqn, ".c") || endswith(fqn, ".cc")) // c or vcc source
		{
			...
			if (is_sdcc)
			{
				...
				fqn = compileFile(fqn);
				source.insertat(
					current_sourceline_index + 1, new SourceLine(q.sourcefile, q.sourcelinenumber, "#local"));
				source.insertat(
					current_sourceline_index + 2, new SourceLine(q.sourcefile, q.sourcelinenumber, "#endlocal"));
			...
			else { throw FatalError("c compiler not set (use cmd line option -c or directive '#cpath')"); }
		}
		else { source.includeFile(fqn, current_sourceline_index + 1); }
	}
}
...
void Z80Assembler::asmTzx(SourceLine& q)
{
	...
	*/
	...
	{
		if (lceq(blocktype, tzx_idf[i]))
		{
			segment_type = tzx_id[i];
			break;
		}
	}
	...
	{						//			[SAMPLE_RATE=value], [CHANNELS=1|2], [MONO], [STEREO],
		...
		while (q.testComma())
		{
			...
			else if (q.testWord("compressed")) { segment->setCompression(true); }
	...
	case TZX_PAUSE: // #tzx PAUSE, [DURATION=]pause
	{
		...
		q.testComma();
		if (q.testWord("duration")) q.expect('=');
	...
	case TZX_LOOP_START: // #tzx LOOP-START, [REPETITIONS=]repetitions
	{
		...
		q.testComma();
		if (q.testWord("repetitions")) q.expect('=');
	...
	case TZX_POLARITY: // #tzx POLARITY, [POLARITY=]polarity
	{
		...
		q.testComma();
		if (q.testWord("polarity")) q.expect('=');
...
void Z80Assembler::asmSegment(SourceLine& q, SegmentType segment_type)
{
	...
	if (segment) // segment definition in pass 2++ or re-enter segment in any pass
	{
		...
		{
			if (q.testComma()) throw FatalError("segment %s redefined", name);
	...
	else // if (!segment)	// create segment in pass 1
	{
		...
		q.label = new Label(
			name, segment, q.sourcelinenumber, 0 /*address*/, invalid, yes /*global*/, yes /*defined*/, l != nullptr);
	...
	do {
		...
		else if (q.testWord("lastbits"))
		{
			...
			if (target != TZX) throw SyntaxError("target TZX required");
			if (segment_type == TZX_STANDARD) throw SyntaxError("TZX pure data, turbo or generalized block required");
...
void Z80Assembler::asmLocal(SourceLine&)
{
	...
	else { assert(labels[local_blocks_count].outer_index == local_labels_index); }
...
void Z80Assembler::asmEndLocal(SourceLine&)
{
	...
	if (pass == 1) // Pass 1: verschiebe undefinierte lokale Label in den umgebenden Kontext
	{
		...
		for (uint lli = local_labels_array.count(); lli--;)
		{
			...
			if (!label->is_defined && !label->is_global) undef_labels_array.append(label);
		}

		// Verschiebe diese Labels in den umgebenden Kontext:
		for (uint uli = undef_labels_array.count(); uli--;)
		{
			Label* ql = undef_labels_array[uli];
			assert(!ql->is_global);
...
void Z80Assembler::asmNoSegmentInstr(SourceLine& q, cstr w)
{
	...
	{
		Macro& m = macros[w];
		if (current_sourceline_index > m.mdef)
		{
			asmMacroCall(q, m);
			return;
		}
	...
	{						   // TODO: testen, ob das Verstellen der aktuellen Zeile Probleme bereitet
		...
		while (++n < e)
		{
			SourceLine& z = source[n];
			z.rewind();
			if (z.testWord(".endme"))
			{
				z.skip_to_eol();
				current_sourceline_index = n;
				goto warn;
			}
		}
	...
	if (lceq(w, "*"))
	{
		...
		if (q.testWord("include"))
		{
			asmInclude(q);
			return;
		} // CAMEL80: fname follows without '"'
	}
...
ignore:
	if (pass > 1 || verbose < 2) return q.skip_to_eol();
	if (0)
	warn:
		if (pass > 1 || verbose < 1) return q.skip_to_eol();
...
void Z80Assembler::asmRawDataInstr(SourceLine& q, cstr w)
{
	...
	{
		CharMap *&p, *v;
		Fin(CharMap*& p) : p(p), v(p) { p = nullptr; }
		~Fin() { p = v; }
	} fin(charset);
	...
	if (doteq(w, "db") || lceq(w, "defb") || doteq(w, "dm") || lceq(w, ".byte") || lceq(w, ".ascii") ||
		lceq(w, ".text"))
	{
...
void Z80Assembler::asmPseudoInstr(SourceLine& q, cstr w)
{
	...
	dl:
		q.is_data = yes;
		do {
			int32 n = value(q).value;
			store(n, n >> 8, n >> 16, n >> 24);
		}
		while (q.testComma());
		return;
	...
	case 'defm':
		...
		dm:
			...
			if (w[0] == '"' || w[0] == '\'')
			{
				...
				cb:
					...
					if (q.testChar('+')) { storeByte(Value(s->popLastByte()) + value(q)); }
		...
		if (w[0] == '_')
		{
			if (cfeq(w, "__date__", casefold))
			{
				w = datestr(timestamp);
				w += *w == ' ';
				goto cb;
			}
			if (cfeq(w, "__time__", casefold))
			{
				w = timestr(timestamp);
				w += *w == ' ';
				goto cb;
			}
			if (cfeq(w, "__file__", casefold))
			{
				w = q.sourcefile;
				goto cb;
			}
			if (cfeq(w, "__line__", casefold))
			{
				w = tostr(q.sourcelinenumber);
				goto cb;
			}
		}
	...
	default: return asmNoSegmentInstr(q, w);
	}
...
longer:
	...
	if (lceq(w, ".expect")) // Test Segment: set expected values
	{
		...
		if (currentPosition().value == 0)
			throw SyntaxError(".expect pseudo instructions must appear after the test code");
		...
		if (q.testWord("cc")) // cycle counter: cc [=,<=,>=] nnnnn
		{
			...
			else { segment->setExpectedCc(&q, cc); }
		}
	...
	{
		if (charset &&
			charset->get(' ', ' ') == 0) // ZX80/81: the only conversion i know where 0x00 is a printable char
...
void Z80Assembler::parseBytes(SourceLine& q, Array<uint8>& dest)
{
	...
	cb:
		while (*w) dest.append(charcode_from_utf8(w));

		// test for operation on the final char:
		if (q.testChar('+')) { dest.append(validatedByte(Value(dest.pop()) + value(q))); }
		else if (q.test_char('-')) { dest.append(validatedByte(Value(dest.pop()) - value(q))); }
		else if (q.test_char('|')) { dest.append(validatedByte(Value(dest.pop()) | value(q))); }
		else if (q.test_char('&')) { dest.append(validatedByte(Value(dest.pop()) & value(q))); }
		else if (q.test_char('^')) { dest.append(validatedByte(Value(dest.pop()) ^ value(q))); }
		return;
