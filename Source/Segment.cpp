...
bool isData(SegmentType t) { return t == DATA; }
...
bool isTest(SegmentType t) { return t == TEST; }

static void check_validity(cValue& old, cValue& nju, cstr name) // helper
{
	if (old.is_valid() && nju.is_valid() && old != nju) throw SyntaxError("%s: value redefined", name);
	if (nju.validity < old.validity) throw SyntaxError("%s: value decayed", name);
}
...
{
	check_validity(old, nju, name);
	if (nju.is_valid() && nju.value != uint16(nju)) throw SyntaxError("%s: value not in range[0 .. $ffff]", name);
}
...
*/
DataSegment::DataSegment(cstr _name) : Segment(DATA, _name), relocatable(yes), resizable(yes), address(), size(), lpos()
{
	is_data = yes;
}
...
void DataSegment::setAddress(cValue& new_address)
{
	...
	{
		if (address.is_valid() && address != new_address) { throw SyntaxError("segment %s address redefined", name); }
	...
	if (new_address.validity < address.validity) throw SyntaxError("segment %s address decayed", name);
...
void DataSegment::setSize(cValue& newsize)
{
	...
	if (newsize.is_valid())
	{
		if (size.is_valid() && size != newsize) throw SyntaxError("segment %s size redefined", name);
	...
	if (newsize.validity < size.validity) throw SyntaxError("segment %s size decayed", name);
...
void DataSegment::setOrigin(cValue& address)
{
	...
	if (address.is_valid())
	{
		if (address.value < -0x8000 || address.value > 0xFFFF) throw SyntaxError("address out of range");
	}
...
void Segment::storeOffset(cValue& offset)
{
	if (offset.is_valid())
	{
		if (int16(offset) != int8(offset)) throw SyntaxError("offset value out of range");
	}
	store(offset.value);
}
...
void Segment::storeByte(cValue& byte)
{
	...
	if (byte.is_valid())
	{
		if (int16(byte) < -0x100 || int16(byte) > 0xFF) throw SyntaxError("byte value out of range");
	}
	store(byte.value);
}
...
void Segment::storeWord(cValue& word)
{
	...
	if (word.is_valid())
	{
		if (word.value < -0x10000 || word.value > 0xFFFF) throw SyntaxError("word value out of range");
	}
	store(word.value);
	store(word.value >> 8);
}
...
void CodeSegment::setFlag(cValue& v)
{
	...
	if (no_flagbyte) throw SyntaxError("flag is already set to 'NONE'");

	if (v.is_valid())
	{
		if (v.value != uint8(v)) throw SyntaxError("value out of range");

		if (flag.is_valid() && v.value != flag.value) throw SyntaxError("segment %s flag redefined", name);
	}
	else
	{
		if (v.validity < flag.validity) throw SyntaxError("segment %s flag decayed", name);
	}
...
{
	if (has_flag && !no_flagbyte) throw SyntaxError("flag is already set");
...
void CodeSegment::setNoChecksum() { no_checksum = yes; }

void CodeSegment::setPause(cValue& v)
{
	if (v.is_valid())
	{
		if (v.value != uint16(v)) throw SyntaxError("value out of range");

		if (pause.is_valid() && v.value != pause.value) throw SyntaxError("pause redefined");
	}
	else
	{
		if (v.validity < pause.validity) throw SyntaxError("value for pause decayed");
	}
...
void CodeSegment::setLastBits(cValue& v)
{
	...
	if (v.is_valid())
	{
		if (v.value < 1 || v.value > 8) throw SyntaxError("value must be in range 1 .. 8");

		if (lastbits.is_valid() && v.value != lastbits.value) throw SyntaxError("lastbits redefined");
	}
	else
	{
		if (v.validity < lastbits.validity) throw SyntaxError("value for lastbits decayed");
	}
...
Validity CodeSegment::validity() const
{
	...
	if (has_flag && v != invalid)
	{
		...
		for (uint i = 0; i < pilotsym.count() && v != invalid; i++) v = min(v, pilotsym[i].validity());

		for (uint i = 0; i < datasym.count() && v != invalid; i++) v = min(v, datasym[i].validity());
	}
...
void CodeSegment::check_pilot_symbol(uint idx) const
{
	...
	const Array<Value>& symbol = pilotsym[idx];

	if (type == TZX_TURBO)
	{
		if (idx >= 2) throw SyntaxError("too many pilot symbols: exactly 2 symbols required for TZX turbo blocks");
	...
	{
		cValue& v = symbol[i];
		if (v.is_valid() && v.value < 100) throw SyntaxError("tzx-pilot-sym[%u][%u]: pulse too short", idx, i);
		if (v.is_valid() && v.value > 0xffff) throw SyntaxError("tzx-pilot-sym[%u][%u]: pulse too long", idx, i);
	}
}
...
void CodeSegment::check_data_symbol(uint idx) const
{
	...
	const Array<Value>& symbol = datasym[idx];
	...
	if (type == TZX_TURBO || type == TZX_PURE_DATA)
	{
		...
		if (symbol.count() != 3) throw SyntaxError("tzx-data-sym[%u]: 3 values required: toggle type + 2 pulses", idx);
	...
	{
		cValue& v = symbol[i];
		if (v.is_valid() && v.value < 100) throw SyntaxError("tzx-data-sym[%u][%u]: pulse too short", idx, i);
		if (v.is_valid() && v.value > 0xffff) throw SyntaxError("tzx-data-sym[%u][%u]: pulse too long", idx, i);
	}
}
...
void CodeSegment::setPilot(Values symbol)
{
	...
	if (type == TZX_TURBO)
	{
		if (symbol.count() != 4 || (symbol[0].value != 0 && symbol[0].is_valid()) ||
			(symbol[2].value != 1 && symbol[2].is_valid()) || (symbol[3].value != 1 && symbol[3].is_valid()))
			throw SyntaxError("TZX turbo block: exactly 4 values required: 0, pilot_count, 1, 1");
	}
	else // GENERALIZED
	{
		if (symbol.count() & 1) throw SyntaxError("pilot symbols must be defined in pairs: symbol_index + repetitions");
	}

	if (pilot.count()) // compare to old:
	{
		if (symbol.count() != pilot.count()) throw SyntaxError("pilot redefined");

		for (uint i = 0; i < pilot.count(); i++)
		{
			Value v(symbol[i]), p(pilot[i]);
			if (p != v && p.is_valid() && v.is_valid()) throw SyntaxError("pilot redefined");
			if (v.validity < p.validity) throw SyntaxError("tzx-pilot[%u]: value decayed", i);
		}
	}
	...
	for (uint i = 0; i < symbol.count();) // validate values:
	{
		...
		if (w.is_valid() && uint(w) > 0xffffu) throw SyntaxError("tzx-pilot[%u]: pulse length out of range", i - 1);
	}
...
void CodeSegment::setNumPilotPulses(cValue& v)
{
	...
	if (v.value == 0 && v.is_valid())
		throw SyntaxError(
			type == CODE || type == TZX_GENERALIZED ?
...
Validity TzxPureToneSegment::validity() const { return min(num_pulses.validity, pulse_length.validity); }
...
Validity TzxCswRecording::validity() const { return (pause + first_frame + last_frame + header_size).validity; }
...
IoSequence::IoSequence(IoSequence&& q) noexcept : data(q.data), count(q.count), repetitions(q.repetitions)
{
	new (&q) IoSequence();
}
...
IoList::IoList(IoMode iomode, cstr filename, uint blocksize) : iomode(iomode), fd(-1, filename), blocksize(blocksize)
{
...
void IoList::openFile() // befor test
{
	switch (iomode)
	{
	case IoStdIn: fd = FD::_stdin; break;
	case IoStdOut: fd = FD::_stdout; break;
	case IoValues:
		sequence_idx	= 0;
		in_sequence_idx = 0;
		repetition		= 0;
		break;
	case IoInFile:
	case IoCompareFile: fd.open_file_r(fd.filepath()); break;
	case IoOutFile: fd.open_file_w(fd.filepath()); break;
	case IoAppendFile: fd.open_file_a(fd.filepath()); break;
	case IoBlockDevice:
...
bool IoList::isAtEnd() // after test
{
	...
	default: return yes;
	case IoInFile:
	case IoCompareFile: return fd.is_at_eof();
	case IoValues:
...
uint8 IoList::inputByte() // during test
{
	...
	case IoOutFile:
	case IoAppendFile:
	case IoCompareFile: IERR();
	case IoStdIn: return fd.data_available() ? fd.read_uchar() : 0;
	case IoInFile: return fd.read_uchar();
	case IoValues:
		while (sequence_idx < data->count())
		{
			IoSequence& seq = (*data)[sequence_idx];
...
void IoList::outputByte(uint8 c, uint8* core) // during test
{
	...
	case IoOutFile:
	case IoAppendFile:
	case IoStdOut: fd.write_uchar(c); return;
	case IoCompareFile:
	{
	...
	case IoBlockDevice:
	{
		...
		default:
			if (c == 0xE5) // read
			{
				if (memory_address + blocksize > 0x10000) throw AnyError("block end beyond ram end");
...
TestSegment::TestSegment(cstr _name, uint8 _fillbyte) : CodeSegment(_name, TEST, _fillbyte)
{
	is_code = false;
	is_test = true;
}
...
void TestSegment::setExpectedRegisterValue(SourceLine* q, cstr regname, Value v)
{
	...
	if (max == 0) throw SyntaxError("%s: not a register name", regname);
...
static uint16 validated_io_address(cValue& addr) // helper
{
	...
	if (addr.value != uint16(addr)) throw SyntaxError("address not in range[0 .. $ffff]");
	return uint16(addr);
}
...
void TestSegment::setBlockDevice(cValue& ioaddr, cstr filename, cValue& blocksize)
{
	...
	if (!blocksize.is_valid()) throw SyntaxError("blocksize must be valid in pass 1");
	if (blocksize.value < 1 || blocksize.value > 0x8000) throw SyntaxError("invalid block size");
	...
	if (outputdata.contains(addr))
	{
		...
		if (list.iomode != IoBlockDevice)
			throw SyntaxError("out($%04x): mode is already set to %s", addr, ioModeNames[list.iomode]);
		if (ne(list.fd.filename(), filename)) throw SyntaxError("in($%04x): block file redefined", addr);
		if (uint(blocksize) != list.blocksize) throw SyntaxError("in($%04x): block size redefined", addr);
		return;
	}
...
void TestSegment::setInputFile(cValue& ioaddr, cstr filename, IoMode mode)
{
	...
	if (inputdata.contains(addr))
	{
		...
		if (list.iomode != mode)
			throw SyntaxError("in($%04x): mode is already set to %s", addr, ioModeNames[list.iomode]);
		if (ne(list.fd.filename(), filename)) throw SyntaxError("in($%04x): input file redefined", addr);
		return;
	}
...
void TestSegment::setOutputFile(cValue& ioaddr, cstr filename, IoMode mode)
{
	...
	if (outputdata.contains(addr))
	{
		...
		if (list.iomode != mode)
			throw SyntaxError("out($%04x): mode is already set to %s", addr, ioModeNames[list.iomode]);
		if (ne(list.fd.filename(), filename)) throw SyntaxError("out($%04x): output file redefined", addr);
		return;
	}
...
{
	for (uint i = 0; i < inputdata.count(); i++) { inputdata.getItems()[i].closeFile(); }
	for (uint i = 0; i < outputdata.count(); i++) { outputdata.getItems()[i].closeFile(); }
}
...
void TestSegment::checkAllBytesRead() // after test
{
	...
	for (uint i = 0; i < inputdata.count(); i++)
	{
		if (!inputdata.getItems()[i].isAtEnd()) e = catstr(e, usingstr(",$%04x", inputdata.getKeys()[i]));
	}
	if (!e) return; // ok
...
void TestSegment::checkAllBytesWritten() // after test
{
	...
	for (uint i = 0; i < outputdata.count(); i++)
	{
		if (!outputdata.getItems()[i].isAtEnd()) e = catstr(e, usingstr(",$%04x", outputdata.getKeys()[i]));
	}
	if (!e) return; // ok
...
DataSegments::DataSegments(const Segments& segments)
{
	// extract Code- and DataSegments from source

	for (uint i = 0; i < segments.count(); i++)
	{
		if (auto s = dynamic_cast<DataSegment*>(segments[i].ptr())) { append(s); }
	}
}

CodeSegments::CodeSegments(const Segments& segments)
{
...
{
	uint32 sz = 0;
	for (uint i = 0; i < count(); i++) { sz += data[i]->outputSize(); }
	return sz;
}
...
TzxSegments::TzxSegments(const Segments& segments)
{
	for (uint i = 0; i < segments.count(); i++)
	{
		if (auto s = dynamic_cast<TzxSegment*>(segments[i].ptr())) { append(s); }
	}
}

TestSegments::TestSegments(const Segments& segments)
{
	for (uint i = 0; i < segments.count(); i++)
	{
