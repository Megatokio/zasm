...
int Z80Assembler::getRegister(SourceLine& q, Value& n, bool with_qreg)
{
	...
	char c1 = *w++ | 0x20;
	if (c1 == 0x20) throw SyntaxError("unexpected end of line");
	char c2 = *w++ | 0x20;
	...
	if (c2 == 0x20) // strlen=1
	{
		...
		case '(':
		{
			...
			if (q.testWord("ix"))
			{
				if (target_8080) goto no_8080;
				r = XIX;
				if (q.testChar(')')) return r;
			}
			if (q.testWord("iy"))
			{
				if (target_8080) goto no_8080;
			...
			n = value(q);
			if (r != XNN && n.value != int8(n) && n.is_valid()) throw SyntaxError("offset out of range");
			q.expectClose();
			return r;
		}
		}
...
void Z80Assembler::asmZ80Instr(SourceLine& q, cstr w)
{
	...
	case '  jr':
		...
		if (r2 > CY) throw SyntaxError("illegal condition");
		instr = r2 == NIX ? JR : JR_NZ + (r2 - NZ) * 8;
		goto jr;
	...
	inc:
		...
		if (r == XIX)
		{
			store(PFX_IX, instr, n.value);
			return;
		} // note: offset tested by getRegister()
		if (r == XIY)
		{
			store(PFX_IY, instr, n.value);
			return;
		} // ""
		if (r <= XL)
		{
			store(PFX_IX, instr + (r + RH - XH - XHL) * 8);
			return;
		}
		if (r <= YL)
		{
			store(PFX_IY, instr + (r + RH - YH - XHL) * 8);
			return;
		}
	...
	case '  ex':
		...
		if (r == HL)
		{
			if (r2 == DE) return store(EX_DE_HL);
			if (r2 == XSP) return store(EX_HL_xSP);
			goto ill_source;
		}
		if (r == DE)
		{
			if (r2 == HL) return store(EX_DE_HL);
			goto ill_source;
		}
		if (r == IX)
		{
			if (r2 == XSP) return store(PFX_IX, EX_HL_xSP);
			goto ill_source;
		}
		if (r == IY)
		{
			if (r2 == XSP) return store(PFX_IY, EX_HL_xSP);
			goto ill_source;
		}
		if (r == XSP)
		{
	...
	case ' add':
		...
		{
		addhl:
			return store(ADD_HL_BC + (r2 - BC) * 16);
		}
		if (r == IX)
		{
			if (r2 == HL) goto ill_source;
			if (r2 == r) r2 = HL;
			store(PFX_IX);
			goto addhl;
		}
	...
	cp_a:
		...
		if (r == NN)
		{
			store(instr + CP_N - CP_B);
			storeByte(n);
			return;
		}
		if (r == XIX)
		{
			store(PFX_IX, instr + XHL - RB, n.value);
			return;
		}
		if (r == XIY)
		{
			store(PFX_IY, instr + XHL - RB, n.value);
			return;
		}
		if (r <= XL)
		{
			store(PFX_IX, instr + r + RH - XH - RB);
			return;
		}
		if (r <= YL)
		{
			store(PFX_IY, instr + r + RH - YH - RB);
			return;
		}
		if (r == XHLPP)
		{
			store(instr + XHL - RB, INC_HL);
			return;
		}
		if (r == XMMHL)
		{
			store(DEC_HL, instr + XHL - RB);
			return;
		}
		goto ill_source;
	...
	case '  ld':
		...
		ld_iy:
			if (r2 == NN)
			{
				store(instr, LD_HL_NN);
				storeWord(n2);
				return;
			}
			if (r2 == XNN)
			{
				store(instr, LD_HL_xNN);
				storeWord(n2);
				return;
			}
			if (r2 == BC)
			{
				if (target_z180) goto ill_opcode;
				return store(instr, LD_H_B, instr, LD_L_C);
			}
			if (r2 == DE)
			{
				if (target_z180) goto ill_opcode;
				return store(instr, LD_H_D, instr, LD_L_E);
			}
			goto ill_source;
		...
		case HL:
			...
			if (r2 == XIX)
			{
				store(PFX_IX, LD_L_xHL, n2.value);
				store(PFX_IX, LD_H_xHL, n2.value + 1);
				return;
			} // ignored: n2+1 may wrap!
			if (r2 == XIY)
			{
				store(PFX_IY, LD_L_xHL, n2.value);
				store(PFX_IY, LD_H_xHL, n2.value + 1);
				return;
			} // ignored: n2+1 may wrap!
			goto ill_source;
		...
		case BC:
			...
			if (r2 == XHL)
			{
				store(LD_C_xHL, INC_HL, LD_B_xHL, DEC_HL);
				return;
			}
			if (r2 == XIX)
			{
				store(PFX_IX, LD_C_xHL, n2.value);
				store(PFX_IX, LD_B_xHL, n2.value + 1);
				return;
			} // ignored: n2+1 may wrap!
			if (r2 == XIY)
			{
				store(PFX_IY, LD_C_xHL, n2.value);
				store(PFX_IY, LD_B_xHL, n2.value + 1);
				return;
			} // ignored: n2+1 may wrap!
			if (r2 == XHLPP)
			{
				store(LD_C_xHL, INC_HL, LD_B_xHL, INC_HL);
				return;
			}
			if (r2 == XMMHL)
			{
				store(DEC_HL, LD_B_xHL, DEC_HL, LD_C_xHL);
				return;
			}
			goto ill_source;
		...
		case DE:
			...
			{
				if (target_z180) goto ill_opcode;
				store(PFX_IX, LD_D_H, PFX_IX, LD_E_L);
				return;
			}
			if (r2 == IY)
			{
				if (target_z180) goto ill_opcode;
				store(PFX_IY, LD_D_H, PFX_IY, LD_E_L);
				return;
			}
			if (r2 == XHL)
			{
				store(LD_E_xHL, INC_HL, LD_D_xHL, DEC_HL);
				return;
			}
			if (r2 == XIX)
			{
				store(PFX_IX, LD_E_xHL, n2.value);
				store(PFX_IX, LD_D_xHL, n2.value + 1);
				return;
			} // ignored: n2+1 may wrap!
			if (r2 == XIY)
			{
				store(PFX_IY, LD_E_xHL, n2.value);
				store(PFX_IY, LD_D_xHL, n2.value + 1);
				return;
			} // ignored: n2+1 may wrap!
			if (r2 == XHLPP)
			{
				store(LD_E_xHL, INC_HL, LD_D_xHL, INC_HL);
				return;
			}
			if (r2 == XMMHL)
			{
				store(DEC_HL, LD_D_xHL, DEC_HL, LD_E_xHL);
				return;
			}
			goto ill_source;
		...
		case SP:
			...
			if (r2 == IX)
			{
				store(PFX_IX, LD_SP_HL);
				return;
			}
			if (r2 == IY)
			{
				store(PFX_IY, LD_SP_HL);
				return;
			}
			if (r2 == NN)
			{
		...
		ld_xiy:
			if (r2 <= RA && r2 != XHL)
			{
				store(instr, LD_xHL_B + r2 - RB, n.value);
				return;
			}
			if (r2 == NN)
			{
				store(instr, LD_xHL_N, n.value);
				storeByte(n2);
				return;
			}
			if (r2 == HL)
			{
				store(instr, LD_xHL_L, n.value);
				store(instr, LD_xHL_H, n.value + 1);
				return;
			} // ignored: n+1 may wrap!
			if (r2 == DE)
			{
				store(instr, LD_xHL_E, n.value);
				store(instr, LD_xHL_D, n.value + 1);
				return;
			} // ignored: n+1 may wrap!
			if (r2 == BC)
			{
				store(instr, LD_xHL_C, n.value);
				store(instr, LD_xHL_B, n.value + 1);
				return;
			} // ignored: n+1 may wrap!
			goto ill_source;
		...
		case XHL:
			...
			if (r2 == BC)
			{
				store(LD_xHL_C, INC_HL, LD_xHL_B, DEC_HL);
				return;
			}
			if (r2 == DE)
			{
				store(LD_xHL_E, INC_HL, LD_xHL_D, DEC_HL);
				return;
			}
			goto ill_source;
		...
		case XNN:
			...
			for (;;)
			{
				if (int8(r2) == RA) { store(LD_xNN_A); }
				else if (int8(r2) == HL) { store(LD_xNN_HL); }
				else if (int8(r2) == IX) { store(PFX_IX, LD_xNN_HL); }
				else if (int8(r2) == IY) { store(PFX_IY, LD_xNN_HL); }
		...
		case XMMHL:
			...
			if (r2 <= RA && r2 != XHL)
			{
				store(DEC_HL, LD_xHL_B + (r2 - RB));
				return;
			}
			if (r2 == BC)
			{
				store(DEC_HL, LD_xHL_B, DEC_HL, LD_xHL_C);
				return;
			}
			if (r2 == DE)
			{
				store(DEC_HL, LD_xHL_D, DEC_HL, LD_xHL_E);
				return;
			}
			if (r2 == NN)
			{
				store(DEC_HL, LD_xHL_N);
				storeByte(n2);
				return;
			}
			goto ill_source;
		...
		case XHLPP:
			...
			if (r2 <= RA && r2 != XHL)
			{
				store(LD_xHL_B + (r2 - RB), INC_HL);
				return;
			}
			if (r2 == BC)
			{
				store(LD_xHL_C, INC_HL, LD_xHL_B, INC_HL);
				return;
			}
			if (r2 == DE)
			{
				store(LD_xHL_E, INC_HL, LD_xHL_D, INC_HL);
				return;
			}
			if (r2 == NN)
			{
		...
		case RA:
			...
			if (r2 == RI)
			{
				storeEDopcode(LD_A_I);
				return;
			}
			if (r2 == RR)
			{
				storeEDopcode(LD_A_R);
				return;
			}
			if (r2 == XBCPP)
			{
				store(LD_A_xBC, INC_BC);
				return;
			}
			if (r2 == XDEPP)
			{
				store(LD_A_xDE, INC_DE);
				return;
			}
			if (r2 == XMMBC)
			{
				store(DEC_BC, LD_A_xBC);
				return;
			}
			if (r2 == XMMDE)
			{
				store(DEC_DE, LD_A_xDE);
				return;
			}
			goto ld_r;
		...
		ld_r:
			...
			if (r2 == XIX)
			{
				store(PFX_IX, LD_B_xHL + (r - RB) * 8, n2.value);
				return;
			}
			if (r2 == XIY)
			{
				store(PFX_IY, LD_B_xHL + (r - RB) * 8, n2.value);
				return;
			}
			if (r2 == XH || r2 == XL)
			{
				store(PFX_IX, LD_B_H + (r2 - XH) + (r - RB) * 8);
				return;
			}
			if (r2 == YH || r2 == YL)
			{
				store(PFX_IY, LD_B_H + (r2 - YH) + (r - RB) * 8);
				return;
			}
			if (r2 == XHLPP)
			{
				store(LD_B_xHL + (r - RB) * 8, INC_HL);
				return;
			}
			if (r2 == XMMHL)
			{
				store(DEC_HL, LD_B_xHL + (r - RB) * 8);
				return;
			}
			goto ill_source;

		case NN: goto ill_dest;
			...
			if (r >= 256) // goodie:  ld rrrr,NNNN  or  ld rrrr,(NN)
			{
				if (r2 == XNN)
				{
					do {
						// loop 1: load lo word
						// loop 2: load hi word
						if (int8(r) == HL) { store(LD_HL_xNN); }
						else if (int8(r) == BC) { storeEDopcode(LD_BC_xNN); }
				...
				if (r2 == NN)
				{
					do {
						// loop 1: load lo word
						// loop 2: load hi word
						if (int8(r) == HL) { store(LD_HL_NN); }
						else if (int8(r) == BC) { store(LD_BC_NN); }
						else if (int8(r) == DE) { store(LD_DE_NN); }
	...
	case ' slp':
		if (target_z180) return store(PFX_ED, 0x76);
		goto ill_z180;
	case 'otim':
		if (target_z180) return store(PFX_ED, 0x83);
		goto ill_z180;
	case 'otdm':
		if (target_z180) return store(PFX_ED, 0x8b);
		goto ill_z180;
	...
	case ' in0':
		...
		{
			r = getRegister(q, n);
			if (r > RA || r == XHL) goto ill_dest;
		}
		q.expectComma();
		r2 = getRegister(q, n2);
		if (r2 == XNN)
		{
			storeEDopcode(0x00 + 8 * (r - RB));
			storeByte(n2);
			return;
		}
		goto ill_source;
...
ill_target:
	if (depp) q.p = depp;
	throw SyntaxError("illegal target"); // 1st arg
ill_source:
	throw SyntaxError("illegal source"); // 2nd arg
ill_dest:
	if (depp) q.p = depp;
	throw SyntaxError("illegal destination"); // jp etc., ld, in, out: destination
ill_z180:
	throw SyntaxError("z180 opcode (use option --z180)");
ill_8080:
	throw SyntaxError("no 8080 opcode (option --8080)");
ill_opcode:
	throw SyntaxError("illegal opcode (option --z180)");
