...
int Z80Assembler::get8080Register(SourceLine& q)
{
	...
	cstr w	= q.nextWord();
	char c	= *w;
	if (c == 0) throw SyntaxError("unexpected end of line");
...
int Z80Assembler::get8080WordRegister(SourceLine& q, uint what)
{
	...
	char c1 = *w++ | 0x20;
	if (c1 == 0) throw SyntaxError("unexpected end of line");
	char c2 = *w++ | 0x20;
...
void Z80Assembler::asm8080Instr(SourceLine& q, cstr w)
{
	...
	case ' sta':
		instr = LD_xNN_A;
		goto iw; // 8080: sta NN  => ld (NN),a

	iw:
		n = value(q); // before store wg. '$'
		store(instr);
		storeWord(n);
		return;
	...
	case ' cmp':
		instr = CP_B;
		goto cmp; // 8080: cmp r => cp  a,r		b c d e h l m a
	...
	case ' dcr':
		instr = DEC_B;
		goto dcr; // 8080: dcr r => dec r			b c d e h l m a
	...
	ill_source:
		throw SyntaxError("illegal source");
	ill_8080:
		throw SyntaxError("no 8080 opcode (use option --asm8080 and --z80)");


	case 'ldax': instr = LD_A_xBC; goto stax; // 8080: ldax r => ld a,(rr)	b=bc d=de
	case 'stax':
		instr = LD_xBC_A;
		goto stax; // 8080: stax r => ld (rr),a	b=bc d=de

	stax:
		r = get8080WordRegister(q, BD);
		return store(instr + r);

	case ' dcx': instr = DEC_BC; goto inx; // 8080: dcx r => dec rr		b, d, h, sp
	case ' inx':
		instr = INC_BC;
		goto inx; // 8080: inx r => inc rr		b, d, h, sp
	...
	case ' pop':
		instr = POP_BC;
		goto pop; // pop  r => pop  rr			b d h psw
