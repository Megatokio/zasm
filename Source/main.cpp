...
static cstr compiledatestr()
{
	...
	uint m = 0;
	while (strncmp(ansidate, months + 3 * m++, 3)) {}
...
static int doit(Array<cstr> argv)
{
	...
	if (argv.count() >= 3 && strchr(argv[1], ' '))
	{
		...
		// argv[] = { "/path/to/zasm", "merged arguments from line 1", "executable/sourcefile/as/invocated", "more",
		// "arguments" } note: a script can start with "#!" (shebang). then the Linux kernel takes everything up to the
		// first space as the command, and everything after that space as ONE SINGLE ARGUMENT.
		try
		{
			FD	 fd(argv[2]);
			cstr s = fd.read_str();
	...
	if (selftest && !compare)
	{
		...
		for (uint i = 0; i < testfiles.count(); i++)
		{
			...
			{
				errors++;
				logline("%s", strerror(errno));
				continue;
			} // error
			char bu[256];
		r:
			int n = int(read(fd, bu, 255)); // read line 1
			if (n < 0)
			{
...
int main(int argc, cstr argv[])
{
	if (argc == 2)
	{
		if (eq(argv[1], "--version"))
		{
			printf(version, compiledatestr());
			return 0;
		}
		if (eq(argv[1], "--help"))
		{
