...
void Z80Assembler::convert8080toZ80(cstr source, cstr dest)
{
	...
	for (uint si = 0; si < zsource.count(); si++)
	{
		...
		case ' pop':
			op = "pop R";
			goto d; // 8080: pop r => pop  rr		b d h psw
		...
		n: // N or NN arg
			...
			zp += np - op;				// 1st part of op
			if (*q == '(') *zp++ = '+'; // arg must not look like "(arg)"
			memcpy(zp, q, size_t(qp - q));
			zp += qp - q;
			q = qp; // arg
			strcpy(zp, np + 1);
			zp += strlen(np + 1); // 2nd part of op
			break;
		...
		case ' mvi':
			r = skip_r(qp);
			goto y; // 8080: mvi r,N => ld r,N		b c d e h l m a
