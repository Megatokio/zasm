...
void Z80Assembler::writeBinFile(FD& fd)
{
	...
	for (uint i = 0; i < segments.count(); i++) { write_segment(fd, segments[i]); }
}
...
void Z80Assembler::writeHexFile(FD& fd)
{
	...
	for (uint i = 0; i < segments.count(); i++)
	{
		...
		uint8*		 wflags = s.compressed ? nullptr : s.core_wflags.getData();
		write_intel_hex(fd, address, data, wflags, size);
...
void Z80Assembler::writeS19File(FD& fd)
{
	...
	// Motorola: "The code/data field may contain any descriptive information identifying the following block of
	// S-records."
	...
	for (uint i = 0; i < segments.count(); i++)
	{
		...
		uint32		 size	= s.outputSizeUpToMaxDpos();
		uint8*		 wflags = s.compressed ? nullptr : s.core_wflags.getData();
		srcount += write_motorola_s19(fd, address, data, wflags, size);
		address += s.outputSize(); // ROM only
	}
...
void Z80Assembler::writeTapFile(FD& fd)
{
	...
	CodeSegments segments(this->segments);
	while (!segments[0]->has_flag)
	{
		assert(segments[0]->size.value == 0);
		segments.remove(0u);
	}
	...
	for (uint i0 = 0, i = 0; i < segments.count(); i0 = i)
	{
		...
		s			   = segments[i];
		do {
			const uint8* qa	  = s->outputData();
			uint32		 size = s->outputSize();
			fd.write_bytes(qa, size);
			for (const uint8* q = qa + size; q > qa;) { checksum ^= *--q; }
		}
		while (++i < segments.count() && !(s = segments[i])->has_flag);
...
void Z80Assembler::writeTzxFile(FD& fd)
{
	...
	for (uint a = i; i < segments.count(); a = i)
	{
		...
		case TZX_PULSES:
		{
			...
			for (uint i = 0; i < s->count; i++) fd.write_uint16_z(uint16(s->pulses[i]));
			i++;
			continue;
		}
		case TZX_CSW_RECORDING: // TODO
		{
			...
			if (s->compressed)
			{
				...
				if (err == Z_OK)
				{
					assert(zlen <= zbu.count());
					zbu.shrink(uint(zlen));
					std::swap(csw, zbu);
				}
				else
				{
		...
		case TZX_HARDWARE_INFO:
		{
			...
			for (uint i = 0; i < s->hwinfo.count(); i++) { fd.write_bytes(&s->hwinfo, 3); }
			i++;
			continue;
		}
		...
		case TZX_GENERALIZED:
		{
			...
			uint npp = 0;
			for (uint i = 0; i < asp; i++) { npp = max(npp, s->pilotsym[i].count() - 1); }
			uint npd = 0;
			for (uint i = 0; i < asd; i++) { npd = max(npd, s->datasym[i].count() - 1); }
		...
		// write data and calc checksum
		i = a;
		s = dynamic_cast<CodeSegment*>(segments[i].ptr());
		do {
			const uint8* qa	  = s->outputData();
			uint32		 size = s->outputSize();
			fd.write_bytes(qa, size);
...
void Z80Assembler::checkTapFile()
{
	...
	if (!segments[0]->has_flag) throw SyntaxError("tape block %s: flag byte missing (argument #4)", segments[0]->name);
...
static Values simple_pilot(int num_pulses) { return Values() << v0 << Value(num_pulses) << v1 << v1; }

static bool is_simple_data_symbol(const Values& symbol)
{
	return symbol.count() == 3 && symbol[0].value == 0 && symbol[1].value == symbol[2].value;
}
static bool is_simple_data_symbols(const Array<Values>& symbols)
{
	return symbols.count() == 0 ||
		   (symbols.count() == 2 && is_simple_data_symbol(symbols[0]) && is_simple_data_symbol(symbols[1]));
}
static bool is_simple_pilot(const Values& pilot)
{
	return pilot.count() == 0 || (pilot.count() == 4 && pilot == simple_pilot(pilot[1].value));
}
static bool is_simple_pilot_symbols(const Array<Values>& symbols)
{
	return symbols.count() == 0 ||
		   (symbols.count() == 2 && (symbols[0].count() == 2 && symbols[0][0].value == 0) && // pilot pulse
			(symbols[1].count() == 3 && symbols[1][0].value == 0));							 // sync pulses
}
static bool is_simple_pilot_scheme(const Values& pilot, const Array<Values>& symbols)
{
	return is_simple_pilot_symbols(symbols) && is_simple_pilot(pilot);
}
static bool is_zxsp_pilot_scheme(const Values& pilot, const Array<Values>& symbols, int flag)
{
...
static bool is_zxsp_data_symbols(const Array<Values>& symbols)
{
...
void Z80Assembler::checkTzxFile()
{
	...
	for (uint i = 0; i < tzxsegments.count(); i++)
	{
		...
		case TZX_PULSES:
		{
			...
			if (s->count == 0 || s->count > 255) throw SyntaxError("TZX pulses: invalid num pulses");
		...
		case TZX_CSW_RECORDING: // TODO
		{
			...
			if (filesize > (1 << 30)) throw SyntaxError("TZX CSW \"%s\": file too long", filename);
			...
			if (eq(ext, ".wav"))
			{
				...
				if (!wf.is_valid)
					throw SyntaxError(
						"TZX CSW \"%s\": unknown wav format: rename file and "
						"set sample-rate, sample-format and channels",
						filename);
			...
			if (s->pause.value < 0 || s->pause.value > 0xffff)
				throw SyntaxError("TZX CSW %s: pause out of range", filename);
		...
		case TZX_PAUSE:
		{
			...
			if (s->duration.value < 0 || s->duration.value > 0xffff) throw SyntaxError("TZX pause: illegal duration");
			break;
		}
		case TZX_GROUP_START:
		{
		...
		case TZX_POLARITY:
		{
			...
			if (s->polarity.value != 0 && s->polarity.value != 1) throw SyntaxError("TZX polarity: illegal value");
			break;
		}
		case TZX_INFO:
		{
	...
	for (uint i = segments.count(); i--;)
	{
		...
		if (!s->has_pause)
			s->setPause(Value(
				s->flag.value & 0x80 ? 2000 : // data block
					s->checksum_ace ? 2 :
									  1000)); // header
		...
		if (s->type == CODE)
		{
			if (is_simple_data_symbols(s->datasym) && is_simple_pilot_scheme(s->pilot, s->pilotsym))
			{
				if (s->no_pilot) s->type = TZX_PURE_DATA;
					...
					s->type =
						is_zxsp_data_symbols(s->datasym) && is_zxsp_pilot_scheme(s->pilot, s->pilotsym, s->flag.value) ?
							TZX_STANDARD :
							TZX_TURBO;
			}
			else { s->type = TZX_GENERALIZED; }
		}
		...
		try
		{
			...
			if (s->type == TZX_STANDARD)
			{
				...
				if (!is_zxsp_pilot_scheme(s->pilot, s->pilotsym, s->flag.value))
					throw SyntaxError("invalid pilot pulses");
			}
			...
			else if (s->type == TZX_GENERALIZED)
			{
				...
				if (n == 0 && !(size == 0 && s->no_flagbyte && s->no_checksum)) set_default_data_symbols(s);
				...
				{
					if (is_simple_pilot(s->pilot)) set_default_pilot(s);
...
void Z80Assembler::checkBinFile() { CodeSegments(segments).checkNoFlagsSet(); }
...
void Z80Assembler::checkSnaFile()
{
	...
	CodeSegment& hs = segments[i0];
	if (hs.compressed) throw SyntaxError("target SNA: header segment %s cannot be compressed", hs.name);
	if (hs.size.value != 27)
		throw SyntaxError("target SNA: header segment %s must be 27 bytes (size=%u)", hs.name, uint(hs.size));
	SnaHead* head = SnaHeadPtr(hs.getData());

	// verify some values from header:
	if ((head->i >> 6) == 1)
		setError("segment %s: i register must not be in range [0x40 .. 0x7F] (i=0x%02X)", hs.name, head->i);
	if (head->iff2 & ~4) setError("segment %s: iff2 byte must be 0 or 4 (iff2=0x%02X)", hs.name, head->iff2);
	uint16 sp = head->spl + 256 * head->sph;
	if (sp > 0 && sp < 0x4002)
		setError("segment %s: sp register must not be in range [0x0001 .. 0x4001] (sp=0x%04X)", hs.name, sp);
	if (head->int_mode > 2)
		setError("segment %s: interrupt mode must be in range 0 .. 2 (im=%u)", hs.name, head->int_mode);
	if (head->border > 7)
		setError("segment %s: border color byte must not be in range 0 .. 7 (brdr=%u)", hs.name, head->border);
	...
	for (uint i = i0 + 1; i < segments.count(); i++)
	{
		...
		if (addr > 0x10000)
		{
			setError("segment %s extends beyond ram end (end=0x%05X)", s.name, uint(addr));
			break;
		}
	}
	if (addr < 0x10000) setError("target SNA: total ram size must be 0xC000 bytes (size=0x%04X)", uint(addr - 0x4000));
}
...
void Z80Assembler::checkAceFile()
{
	...
	if (!ramsize_valid)
		setError("total ram size must be 0x2000 (3k), 0x6000 (+16k) or 0xA000 (+32k) (size=%u)", uint(ramsize));
	...
	for (uint i = 0; addr < 0x3C00; i++)
	{
		...
		if (s.address.value != addr)
			setError("segment %s must start at 0x%04X (address=0x%04X)", s.name, uint(addr), uint(s.address));
		if (s.compressed) throw SyntaxError("segment %s cannot be compressed", s.name);
		if (s.size.value & 0x3ff)
			throw SyntaxError("segment %s size must be a multiple of 0x400 (size=0x%04X)", s.name, uint(s.size));
		...
		for (uint32 offs = 0; offs < uint32(s.size) && addr < 0x3C00; offs += 0x400, addr += 0x400)
		{
			...
			case 0x2000: // VRAM mirror with Z80 registers
			{
				...
				if (peek2Z(&head->iff1) > 1)
					setError("segment %s: z80_regs[13].iff1 must be 0 or 1 (iff1=%u)", s.name, head->iff1);
				if (peek2Z(&head->iff2) > 1)
					setError("segment %s: z80_regs[14].iff2 must be 0 or 1 (iff2=%u)", s.name, head->iff2);
				if (peek2Z(&head->i) > 255)
					setError("segment %s: z80_regs[15].reg_i must be in range 0 .. 0xff (i=%u)", s.name, head->i);
				if (peek2Z(&head->r) > 255)
					setError("segment %s: z80_regs[16].reg_r must be in range 0 .. 0xff (r=%u)", s.name, head->r);
				if (peek2Z(&head->flag_80) != 0x80) setError("segment %s: z80_regs[17].flag must be 0x80", s.name);
				break;
			}
			...
			case 0x3800: // Prog RAM 3rd mirror
			{
				...
				if (!empty)
					setError("segment %s: page 0x%04X-0x%04X must be empty", s.name, uint(addr), uint(addr + 0x3ff));
				break;
			}
			default: // Prog RAM
				IERR();
...
void Z80Assembler::checkZX81File()
{
	...
	a:
		...
		if (s.size.value > 128)
			throw SyntaxError("segment %s: program name too long: max=128 bytes (size=%u)", s.name, uint(s.size));
	...
	if (!ramsize_valid) setError("total ram size out of range: must be ≥125+1 ($7D+1) and ≤16k (size=$%04X)", ramsize);

	CodeSegment& hs = segments[hi];
	if (hs.address.value != 0x4009) throw SyntaxError("segment %s: first segment must start at $4009", hs.name);
	if (hs.compressed) throw SyntaxError("segment %s: system variables cannot be compressed", hs.name);
	if (hs.size.value < 125 - 9)
		throw SyntaxError("segment %s must be at least 125-9 ($7D-9) bytes (size=%u)", hs.name, uint(hs.size));
...
void Z80Assembler::checkZ80File()
{
	...
	if (hs.size.value == z80v1len)
	{
		...
		if (head.data != 255 && head.data & 0x20 && compressed_page_size_z80(s.getData(), 0xc000) > 0xc000)
			head.data -= 0x20;
		return;
	}
	...
	for (uint i = i0 + 1; i < segments.count(); i++)
	{
		...
		if (!s.has_flag)
		{
			setError("segment %s: page ID missing", s.name);
			continue;
		}
		...
		case 2: // rom at address 0x4000
			if (!paged_mem) setError("segment %s: invalid page ID: this model does not have 32 kB of rom", s.name);
			goto anypage;
		...
		rampage:
			...
			if (varying_pagesize) // b&w machines:
			{
				if (page_id > 7)
				{
					setError("segment %s: page ID out of range", s.name);
					continue;
				}
				if (loaded & (1 << page_id))
				{
					setError("segment %s: page ID occured twice", s.name);
					continue;
				}
				loaded |= 1 << page_id;
				...
				{
					setError("segment %s: page size does not match page ID", s.name);
					continue;
				}
				if (int(addr) + size > s.size.value)
				{
					setError("segment %s: sum of page sizes exceeds ram size", s.name);
					continue;
				}
				addr += uint(size);
			}
			else // std. 16k page:
			{
				...
				addr += 16 kB;
			anypage:
				if (s.size.value != 16 kB)
				{
					setError("segment %s: page size must be 16 kB", s.name);
					continue;
				}
			}
			continue;
